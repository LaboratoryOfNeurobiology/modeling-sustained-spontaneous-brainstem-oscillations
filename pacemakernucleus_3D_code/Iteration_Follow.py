...
class Iteration(object):
    ...
    def __init__(self, func_name, sol_range, num_div, it_step, nodes_scaled,
                 nodes_discrete, cu, cu_add, no, status=None, next_node_idx=0,
                 isFollowing=False):
    ...
    def save(self):
        ...
        with open(f"{self.file_tree.get_its_num_dir(root, self.it_step, self.isFollowing)}"
                  f"/{file_name}.pkl", 'wb') as f:
            pkl.dump(self, f)
    ...
    def give_shapes(self):
        ...
        """if self.next_node_idx == len(self.nodes_scaled):
            return self.results, self.nodes_discrete, self.No, self.Cu, \
                    self.num_div, self.sol_range, self.it_step
        else:
            print(f"{self.next_node_idx} is not == {len(self.nodes_scaled)}, "
                  f"finish nodes_scaled first.")
            return"""

    def give_cu_add(self):
        return self.Cu_add

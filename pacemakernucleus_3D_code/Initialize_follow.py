...
def initialize_follow(No, Cu, sol_range, num_div, it_step):
    """
    Initializes the solution surface following iteration and outputs

    :param No:  dictionary that contains all discrete nodes and associated frequencies, corresponding to
                interval-halving step it_step
    :param Cu:  a tuple of tuples that contain (discrete) nodes of n-cubes corresponding to iteration step it_step
    ...
    :return:    nodes_scaled:   2D numpy array containing scaled coordinates (columns) of all newly added nodes (rows)
                ...
                Cu_br:  a tuple of tuples that contain (discrete) nodes of all bracketing n-cubes
    """
    ...
    print('    Bracketing cubes have been identified. | Time consumption:', t0_end - t0_start, '[second]')

    # FINDING ALL NEIGHBORING CUBES OF NEW BRACKETING CUBES ------------------------------------------------------------

    t1_start = time.perf_counter()

    # This dictionary stores all nodes that make up the neighboring cubes of bracketing cubes (these nodes may be
    # outside the search range)
    # keys: discrete nodes
    # values: keys of associated cubes in Cu_new
    No_new = {}

    # This dictionary stores all neighboring cubes of bracketing cubes
    # keys: sequence numbers used to identify unique cubes
    # values: tuple of discrete nodes
    Cu_new = {}

    # Collecting all neighboring cubes of bracketing cubes
    key = 0
    for cube in Cu_br:  # checking for each bracketing cube
    ...
    print('    Neighboring cubes of new bracketing cubes and nodes have been collected. | Time consumption:',
    ...
    # Dictionary that contains only nodes that have not been computed yet and do not fall outside the search range
    # keys: discrete nodes
    ...
    print('    New nodes have been collected. | Time consumption:', t2_end - t2_start, '[second]')
    ...
    print('    New cubes have been collected. | Time consumption:', t3_end - t3_start, '[second]')
